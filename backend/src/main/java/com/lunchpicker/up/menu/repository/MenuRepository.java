...
public interface MenuRepository extends JpaRepository<Menu, Long> {
    ...
    @Query("SELECT new com.lunchpicker.up.menu.dto.MenuWithStatsDto(" +
           ...
           "m.createdAt) " +
           "FROM Menu m " +
           "WHERE (:categoryCode IS NULL OR m.categoryCode = :categoryCode) " +
           "AND (:priceRangeCode IS NULL OR m.priceRangeCode = :priceRangeCode) " +
           "AND (:distanceCode IS NULL OR m.distanceCode = :distanceCode) " +
           "ORDER BY m.createdAt DESC")
    List<MenuWithStatsDto> findAllWithStatsAndFilter(
            @Param("categoryCode") String categoryCode,
            @Param("priceRangeCode") String priceRangeCode,
            @Param("distanceCode") String distanceCode
    );
    ...
    Optional<MenuWithStatsDto> findByIdWithStats(@Param("id") Long id);
}
